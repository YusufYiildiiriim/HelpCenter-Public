...
import { X, Edit, Plus, Loader2, Check, Shield, FolderKanban, Info, Users, LayoutGrid, Search, ChevronRight, ChevronLeft } from "lucide-react";
...
const TABS = [
  { key: "info", label: "Proje Bilgileri", icon: <FolderKanban size={14} /> },
  { key: "modules", label: "Modüller", icon: <LayoutGrid size={14} /> },
  { key: "users", label: "Çalışanlar", icon: <Users size={14} /> },
];
...
export const ProjectModal: React.FC<ProjectModalProps> = ({
  ...
  const { register, handleSubmit, validateFields } = useValidatedForm<typeof values>(values, {
    name: { label: "Proje Adı", rules: { required: true } },
  ...
  const filteredModules = allModules.filter((m) =>
    m.name.toLowerCase().includes(moduleSearch.toLowerCase().trim())
  );
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-4 md:p-6 lg:pl-76 lg:pr-6 overflow-y-auto bg-slate-950/70 backdrop-blur-md animate-in fade-in duration-300">
      {/* Backdrop click handler */}
      <div className="fixed inset-0" onClick={onClose} />

      {/* Modal Container */}
      <div className="relative w-full max-w-lg bg-slate-900 rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-800 flex flex-col max-h-[80vh] sm:max-h-[84vh] my-auto overflow-hidden animate-in zoom-in slide-in-from-bottom-4 duration-300 z-10">
          ...
          <div className="flex items-center gap-3 sm:gap-4 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              <h2 className="text-base sm:text-xl md:text-2xl font-black text-white leading-tight truncate">
                {isEditMode ? "Projeyi Düzenle" : "Yeni Proje Tanımla"}
              </h2>
              <p className="text-slate-400 text-[9px] sm:text-[10px] md:text-xs font-bold uppercase tracking-widest mt-0.5 truncate">
                {isEditMode ? "Mevcut yapılandırmayı güncelleyin" : "Sisteme yeni bir proje ekleyin"}
              </p>
            </div>
          </div>
          <button
        ...
        >
          ...
          {activeTab === 0 && (
            <div className="space-y-4 sm:space-y-6 animate-in fade-in slide-in-from-right-2 duration-300">
              {sectionHeader(<FolderKanban size={16} className="text-indigo-400" />, "Proje Bilgileri")}
              ...
              <FormField
                ...
                placeholder="Örn: A Projesi, B Müşteri Grubu..."
              />
              ...
              <FormField
                ...
                required
                icon={<Info size={12} />}
                placeholder="Projenin kapsamını ve amacını belirtin."
              />
              ...
              <div className="p-3.5 sm:p-5 bg-slate-800/60 rounded-xl sm:rounded-2xl border border-slate-700 flex items-center justify-between group">
                <div className="flex items-center gap-3 sm:gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold tracking-tight">Proje şu an {isActive ? 'aktif' : 'pasif'} durumda</p>
                  </div>
                </div>
                <button
                  type="button"
          ...
          {activeTab === 1 && (
            <div className="space-y-3 sm:space-y-4 animate-in fade-in slide-in-from-right-2 duration-300">
              {sectionHeader(<LayoutGrid size={16} className="text-indigo-400" />, "Proje Modülleri", selectedModuleIds.length)}
                ...
                {filteredModules.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-8 gap-2 opacity-40">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                      {allModules.length === 0 ? "Tanımlı modül bulunamadı." : "Aramanıza uygun modül yok."}
                    </p>
                  </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                        ...
                        <p className={cn(
                          "text-[10px] sm:text-[11px] font-black uppercase leading-none truncate transition-colors",
          ...
          {activeTab === 2 && (
            <div className="space-y-3 sm:space-y-4 animate-in fade-in slide-in-from-right-2 duration-300">
              {sectionHeader(<Users size={16} className="text-indigo-400" />, "Projede Çalışan Kullanıcılar", selectedUserIds.length)}
          ...
          <div className="flex items-center justify-between gap-2 sm:gap-3 pt-3 border-t border-slate-800 shrink-0">
            ...
            {activeTab < TABS.length - 1 ? (
              ...
              >
                ...
                {loading ? "İşleniyor..." : (isEditMode ? "Değişiklikleri Kaydet" : "Projeyi Tanımla")}
              </button>
            )}
          </div>
        </form>
